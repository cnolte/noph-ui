...
- **ButtonGroup**: a press left the widths alone when a child sat in a
  `display: contents` wrapper, which measures 0 pixels wide. The group now
  measures the button inside the wrapper.
...
- **NativeSelect**: the field now honours `--np-select-min-width` and
  `--np-select-max-width` like `Select`, instead of sizing itself to its
  widest option.
